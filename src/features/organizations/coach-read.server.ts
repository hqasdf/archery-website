...
import { mapSession, type DbSession } from "@/features/sessions/read.server";
...
const COACH_SESSION_SELECT = `
  id,user_id,title,session_date,session_type,arrow_count,created_at,
...
export async function readCoachOrganizationSessions(organizationId: string): Promise<CoachSession[]> {
  ...
  return ((data ?? []) as unknown as (DbSession & { user_id: string })[])
    .map((row) => ({ ...mapSession(row), userId: row.user_id }));
}
