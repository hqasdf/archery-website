...
export function SessionsWorkspace({initialSessions}:{initialSessions:SessionDraft[]}) {
  ...
  async function createRound(replaceRoundId:string|null=null) {
    ...
    const result=await createRoundWithEnds({sessionId:session.id,roundNumber:oldRound?.roundNumber??session.rounds.length+1,...roundForm,distanceMetres:Number(distanceText)}); setPending(false);
    if (!result.ok) {
      if (oldRound) {
  ...
  return <section className={styles.workspace}>
    ...
    {view==="setup"&&session&&<form className={styles.formCard} onSubmit={submitRound}>
      <div className={styles.viewTop}><button type="button" className={styles.textButton} onClick={()=>{setConfiguringRoundId(null);setView(activeRoundId?"scoring":"session");}}>← {activeRoundId?"Scoring":"Session"}</button><p>Round {configuringRoundId?session.rounds.find((round)=>round.id===configuringRoundId)?.roundNumber:session.rounds.length+1}</p></div><h2>Configure Round</h2><p className={styles.formIntro}>Presets are starting points. Adjust anything to match what you are shooting.</p><RoundPresetsPanel selectedId={selectedPreset} onSelect={choosePreset}/>
