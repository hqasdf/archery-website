...
type DbSession={id:string;title:string;session_date:string;session_type:string;arrow_count:number;created_at:string;session_rounds:DbRound[]|null};
...
export async function readSessions(options?:{sessionType?:SessionType;limit?:number}):Promise<SessionDraft[]> {
  ...
  `).eq("user_id",user.id).order("session_date",{ascending:false}).order("created_at",{ascending:false});
...
function mapSession(row:DbSession):SessionDraft {
