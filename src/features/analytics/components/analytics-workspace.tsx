...
import { calculateDistancePerformance, calculateOverview, calculateTargetGroupings, calculateTrend, DEFAULT_ANALYTICS_FILTERS, filterAnalyticsRounds, getAvailableFilters, type AnalyticsDateRange, type AnalyticsFilters, type AnalyticsSessionType, type TrendPoint } from "../analytics-model";
  ...
  return <div className={styles.analytics}>
    ...
    <section aria-labelledby="overview-heading"><SectionHeading eyebrow="Saved performance" heading="Overview" id="overview-heading"/>
      ...
      <div className={styles.bestRound}>
        <div><p className={styles.kicker}>Best completed Round</p>{overview.bestRound ? <><h3>{overview.bestRound.name}</h3><p>{formatDate(overview.bestRound.date)}</p></> : <><h3>No completed Round in this view</h3><p>A Round qualifies after every planned Arrow has been saved.</p></>}</div>
    ...
    <section aria-labelledby="trend-heading"><SectionHeading eyebrow="Normalized comparison" heading="Performance trend" id="trend-heading"/><div className={styles.panel}>{trend.length === 0 ? <EmptyState title="No scored Rounds in this view" body="Change the filters or score a Round to build your trend."/> : <><TrendChart points={trend}/><div className={styles.trendList}>{trend.map((point) => <div key={point.id}><span>{formatDate(point.date)} · {point.roundName}</span><strong>{point.average.toFixed(2)} avg · {point.arrowCount} {point.arrowCount === 1 ? "Arrow" : "Arrows"}</strong></div>)}</div></>}</div></section>
...
function formatDate(value: string) { return new Intl.DateTimeFormat("en-SG", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }).format(new Date(`${value}T00:00:00Z`)); }
...
function TrendChart({ points }: { points: TrendPoint[] }) {
  ...
  return <div className={styles.chartWrap}><svg viewBox="0 0 800 244" role="img" aria-label="Average score per Arrow by Round over time">
    ...
    {points.map((point, index) => <circle key={point.id} cx={x(index)} cy={y(point.average)} r="5" className={styles.trendPoint}><title>{formatDate(point.date)} · {point.roundName} · {point.average.toFixed(2)} average · {point.arrowCount} Arrows</title></circle>)}
  </svg></div>;
}
